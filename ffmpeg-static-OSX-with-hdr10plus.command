...
Processor=$(system_profiler SPHardwareDataType | grep -e VMware -e Intel | cut -d ':' -f2 |  cut -d ' ' -f2-10)
if echo "$Processor" | grep -e VMware -e Intel >/dev/null 2>&1 ; then echo "$Processor" Processor Supported ; else echo "$Processor" Processor not Supported \
...
cd libblura*/
# Bug fix: linker error on duplicated symbol "dec_init". Caused by the naming conflict of libbluray and ffmpeg. Solved by changing function name on source code of libbluray.
sed -i '' 's/dec_init/bd_dec_init/g' "src/libbluray/disc/dec.h" "src/libbluray/disc/dec.c" "src/libbluray/disc/disc.c"
./bootstrap
...
cd harfbuzz
mkdir build && cd build
cmake -G "Ninja" .. -DCMAKE_INSTALL_PREFIX:PATH=${TARGET}
ninja && ninja install
...
./autogen.sh
./configure --prefix=${TARGET} --disable-dependency-tracking --with-ogg-libraries=${TARGET}/lib --with-ogg-includes=/Volumes/RamDisk/sw/include/ --enable-static --disable-shared --build=x86_64
make -j "$THREADS" && make install
...
cd SVT-AV1/Build/
cmake .. -G"Unix Makefiles" -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX:PATH=${TARGET} -DBUILD_SHARED_LIBS=OFF
make -j "$THREADS" && make install
...
cd 12bit
cmake -G "Ninja" ../../../x265*/source -DCMAKE_INSTALL_PREFIX:PATH=${TARGET} -DENABLE_HDR10_PLUS=ON -DHIGH_BIT_DEPTH=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF -DMAIN12=ON
ninja ${MAKEFLAGS}

tput bold ; echo ; echo '📍 ' x265 10bit Build ; tput sgr0 ; sleep 2
cd ../10bit
cmake -G "Ninja" ../../../x265*/source -DCMAKE_INSTALL_PREFIX:PATH=${TARGET} -DENABLE_HDR10_PLUS=ON -DHIGH_BIT_DEPTH=ON -DEXPORT_C_API=OFF -DENABLE_SHARED=OFF -DENABLE_CLI=OFF
ninja ${MAKEFLAGS}
...
export LDFLAGS="-L${TARGET}/lib -Wl,-framework,OpenAL,-ld_classic"
